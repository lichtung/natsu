...
/**
 * Class Builder 查询构造器
 * @package Emanon\Library\Elasticsearch\Builder
 */
abstract class Builder
{
    ...
    {
        if (in_array($name, ['gt', 'gte', 'lt', 'lte',])) {
            $fieldName = $arguments[0] ?? null;
            $fieldValue = $arguments[1] ?? null;
            if (isset($fieldName, $fieldValue)) {

            }
            isset($this->query['range']) or $this->query['range'] = [];
            isset($this->query['range'][$fieldName]) or $this->query['range'][$fieldName] = [];
            $this->query['range'][$fieldName][$name] = $fieldValue;
        }
    }
